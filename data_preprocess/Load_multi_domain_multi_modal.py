...
from config.trainer_config import DATA_INFO_DICT, DATA_ROOT_DIR_DICT

# face_scale = 0.9  #default for test, for training , can be set from [0.8 to 1.0]

# data augment from 'imgaug' --> Add (value=(-40,40), per_channel=True), GammaContrast (gamma=(0.5,1.5))
seq = iaa.Sequential([
    iaa.AdditiveGaussianNoise(scale=(0,0.2*255)), #加噪声
    iaa.AddToBrightness((-30,30)),#调亮度
...
class Normaliztion(object):
    ...
    def __call__(self, sample):
        image_x, image_x_depth, image_x_ir, image_x_ir_HOG, image_x_ir_PLGF, spoofing_label, map_x1,domain = sample['image_x'], \
        ...
        return {'image_x': new_image_x, 'image_x_depth': new_image_x_depth, 'image_x_ir': new_image_x_ir,
                ...
                'spoofing_label': spoofing_label, 'map_x1': map_x1,'domain':domain}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image_x': torch.from_numpy(image_x.astype(float)).float(),
                'image_x_depth': torch.from_numpy(image_x_depth.astype(float)).float(),
                'image_x_ir': torch.from_numpy(image_x_ir.astype(float)).float(),
                'image_x_ir_HOG': torch.from_numpy(image_x_ir_HOG.astype(float)).float(),
                'image_x_ir_PLGF': torch.from_numpy(image_x_ir_PLGF.astype(float)).float(),
                'spoofing_label': torch.from_numpy(spoofing_label_np.astype(np.int_)).long(),
                'map_x1': torch.from_numpy(map_x1.astype(float)).float(),
...
class Multi_Domain_Spoofing_train(Dataset):
    def __init__(self, data_list=['WMCA', 'USC', 'CeFA', 'SURF'], transform=None, get_type='all'):
        ...
        self.root_dir_dict = DATA_ROOT_DIR_DICT
        self.info_dict = DATA_INFO_DICT['train']
    ...
    def load_SURF(self, info_list):
        """CASIA_SURF中spoofing_label为0的是假脸，为1的是真脸"""
        landmarks_frame = pd.read_csv(info_list, delimiter=' ', header=None)
        for idx in range(len(landmarks_frame)):
            video_name = str(landmarks_frame.iloc[idx, 0])
...
class Multi_Domain_Spoofing_valtest(Dataset):
    ...
    def __init__(self, data_list=['WMCA', 'USC', 'CeFA', 'SURF'], transform=None, get_type='all'):
        ...
        self.root_dir_dict = DATA_ROOT_DIR_DICT
        self.info_dict = DATA_INFO_DICT['test']
        self.transform = transform
        # print(len(self.landmarks_frame))
        self.dir_list = []
        """仿照single-side 每个domain的假是一类，所有true是一类"""
        self.domain_id = {
            "True": 0,
            "WMCA": 1,
            "CeFA": 2,
...
class Flexible_Multi_Domain_Spoofing_valtest(Dataset):
    def __init__(self, data_list=['WMCA', 'USC', 'CeFA', 'SURF'], transform=None, missing_modal=None, get_type='all'):
    ...
    def load_WMCA(self, info_list):
        ...
        for idx in range(len(landmarks_frame)):
            ...
            if self.get_type == 'neg' and spoofing_label == 1:
                continue
            if self.missing_modal is not None:
                if 'D' in self.missing_modal:
                    depth_path = None
                if 'IR' in self.missing_modal:
                    ir_path = None
                    HOG_ir_path = None
                    PLGF_ir_path = None
...
if __name__ == '__main__':
    x = Multi_Domain_Spoofing_train(data_list=['CeFA', 'USC'])
    print(x.dir_list)
    y = DataLoader(x, batch_size=1)
    for yy in y:
        print(yy)
    # test_WMCA = '/home/hdd1/share/public_data/FAS-2023/WMCA/yzt/P1/WMCA_Grandtest_test.txt'
    # WMCA_root = '/home/hdd1/share/public_data/FAS-2023/WMCA/image/'
    # x = Spoofing_train(test_WMCA, WMCA_root, transform=transforms.Compose([Normaliztion(), ToTensor()]))
    # y = DataLoader(x, batch_size=1)
    # for yy in y:
    #     print(yy)
    pass
